...
image — both started as backlog items here and were pulled into milestones. M12 (a second,
square layout template, locked in at creation time) is scoped and ready — see `docs/CHECKLIST.md`.
Remaining backlog:
...
- A third layout template (poster) once M12 proves the two-template plumbing.
...
| Auto-sorting a combine's tracks chronologically by each GPX's own recorded `<time>` (2026-09-05, `docs/issues/2026-09-05-combined-track-start-finish-middle.md`) | A manual "Order the tracks" reorder step (up/down per row) between selecting tracks and combining them | No new GPX `<time>`-parsing surface (absent from `core:gpx`'s `parseTrack` entirely today), and correct even when one or more tracks carry no `<time>` data at all — the user is the ground truth either way. Root cause was `defaultCombinedLabelRequests` (`WayprintLayout.kt`) anchoring global Start/Finish to the first/last track by list position, fed by `RecentsViewModel.combineSelected`'s raw tap-selection order — easy to get backwards since Recents itself sorts newest-imported-first. |
| `compileOnly(compose.runtime)` declared once in `commonMain` (`configureComposeStabilityMarker()`, M16.1 original) | Same dependency declared per source-set family: `compileOnly` on `androidMain`/`jvmMain`, `api` on `iosMain` | Kotlin/Native doesn't support `compileOnly` resolution the way JVM/Android do — a `commonMain`-level `compileOnly` prints "Unsupported compileOnly Dependencies ... Kotlin/Native" on every build once iOS targets exist. `iosMain` exists as an accessible intermediate source set automatically (Kotlin's default hierarchy template creates it even when only `iosArm64()`/`iosSimulatorArm64()` are declared, with no explicit `ios()` shortcut or manual `dependsOn` wiring) — no such split was needed before M16.1 added those targets. |
| Raw `NSData`/Foundation interop (`writeToFile`, `dataWithContentsOfURL`, ...) for reading/writing arbitrary bytes on iOS (M16.1's `PlatformFileHandle`/`ImageExporter.share`) | `kotlinx-io` (`SystemFileSystem`/`Path`/`Source`/`Sink`, already a project dependency for `core:storage`'s `Path`) | Foundation's Kotlin/Native binding names for these methods don't match their Swift names and have to be discovered by trial compilation on this Linux machine (no Mac to check against real headers); kotlinx-io is uniform across every KMP target already in this project and removes the guesswork. `PHPhotoLibrary`/`UIImage` (`ImageExporter.saveToGallery`) still need raw Foundation/UIKit interop — no kotlinx-io equivalent for the photo library. |
| Loading `commonTest` fixture files via classpath resources (`javaClass.getResourceAsStream(...)`, `core:gpx`'s `GpxParserTest`/`RdpTest`/`RouteArtTest` originally) | The fixture inlined as a `private const val` raw string constant, exposed as a `Source` via `Buffer().apply { writeString(it) }` (kotlinx-io) | Kotlin/Native has no classpath/classloader resource lookup, so a `commonTest` resource file compiles and passes on JVM/Android but silently never got exercised for the iOS test source sets — `compileTestKotlinIosArm64`/`IosSimulatorArm64` failed the moment M16 added iOS targets, undetected until then because M15/M16's own Verify lines only ever compiled iOS *main* sources. A string constant works identically on every target already using kotlinx-io, with no new dependency. |
| `String.toByteArray()` (`kotlin.text`) in `commonTest` code (`core:storage`'s `TracksStorageTest` originally) | `String.encodeToByteArray()` (also `kotlin.text`, UTF-8, genuinely multiplatform) | `toByteArray()` has no Kotlin/Native overload, so it compiled and passed on JVM/Android but failed `compileTestKotlinIosArm64`/`IosSimulatorArm64` the moment M16 added iOS targets — same undetected-gap shape as the `core:gpx` fixture-loading entry directly above, same root cause (M15/M16's Verify lines never compiled iOS *test* sources). `encodeToByteArray()` is a drop-in replacement, no kotlinx-io needed. |
