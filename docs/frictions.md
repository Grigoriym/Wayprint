...
- M5.3 verify: eyeballed the new Export button's tap coordinate from a screenshot (scaling the displayed-image position by the stated 1.2 factor) and missed by ~500px vertically — landed above the button with no visible error, only caught by a follow-up `uiautomator dump` (`bounds="[404,2239][676,2352]"`) showing the real center. Re-derived the tap from the dump instead.
- M5.4 verify: eyeballed a DocumentsUI file-list row's tap coordinate from a screenshot (mixing displayed-image and real-device pixel spaces) and landed on the wrong row (`notgpx.txt` instead of `fixture.gpx`), triggering the app's `Error` state instead of a real bug — re-derived the tap from a `uiautomator dump`'s row `bounds` instead.
