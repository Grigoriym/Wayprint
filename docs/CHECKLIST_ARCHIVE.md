# Wayprint checklist — archive (M0–M11, all done)

Completed milestone steps moved out of `docs/CHECKLIST.md` to keep that file small; each step's
full body (including its `Note:`/`Verify:` lines) is preserved verbatim below for reference.
See `docs/CHECKLIST.md` for the live banner, workflow rules, and the current Backlog.
...
- [x] **M15.8** — Build-logic: add `jvm()` to `configureKmp()` (`KmpConfiguration.kt`), mirroring
  ...
  **Verify:** `./gradlew build` succeeds for the `jvm` target across every module (the whole
  ...
  Note: no separate `jetbrains.compose.desktop` catalog entry was needed — `compose.desktop.currentOs`
  is a DSL accessor from the `org.jetbrains.compose` plugin (already applied by
  `wayprint.kmp.library.compose`), not a version-catalog dependency. `jvmMain.dependencies` also
  needed `kotlinx-coroutines-swing` (new catalog entry) — `WayprintViewModel`/`RecentsViewModel`
  use `viewModelScope` (`Dispatchers.Main.immediate`), which has no implementation on bare JVM
  without it; omitting it would have crashed the app the instant a ViewModel was created, a failure
  no build/lint step catches. `compose.desktop { application { nativeDistributions { ... } } }`
  packages Linux only (`TargetFormat.Deb`, `art/wayprint_logo.png` as the icon) — Windows/macOS
  need `.ico`/`.icns` assets that don't exist yet, and this dev machine can't build or verify
  those installers regardless, so declaring them now would be exactly the kind of speculative,
  unverifiable config CLAUDE.md's simplicity rule warns against. Skipped Taiga's `jvmArgs` heap
  tuning, `modules("jdk.unsupported")`, and the proguard-desktop release block entirely — tuning
  and a native-lib workaround Wayprint has no evidence it needs, and a release-packaging concern
  the user has separately deferred (`docs/frictions.md` / memory: release CI held off).
  **A real portability bug `jvm()` immediately surfaced, unrelated to anything M15.1–M15.7 touched**:
  `composeApp`'s `KoinGraphTest.kt` lived in `commonTest` and imported `android.content.Context`
  directly (for `verify(extraTypes = listOf(Context::class))`) — compiled fine when Android was the
  only real target, but `commonTest` must now also compile for `jvmTest`, where that type doesn't
  exist. Fixed by following `TaigaMobileNova/composeApp/src/jvmTest/.../KoinGraphTest.kt`'s own
  real precedent (not its docs, which describe a much more elaborate manual-resolution version):
  split into `composeApp/src/androidHostTest/.../KoinGraphTest.kt` (unchanged, still asserts
  `Context` is externally supplied) and a new `composeApp/src/jvmTest/.../KoinGraphTest.kt`
  (`verify()` with no `extraTypes` — the JVM graph's `PlatformComponentModule`/`PlatformStorageModule`/
  `PlatformUiModule` actuals need nothing externally supplied). Moved `koin.test` from
  `commonTest.dependencies` to each of those two source sets' own `dependencies` block.
  Physically creating `src/androidHostTest/...` for the first time also exposed a real gap in
  `config/detekt/detekt.yml`: every rule's test-source `excludes` list named `androidUnitTest`
  (a name that source set never actually has here — `com.android.kotlin.multiplatform.library`'s
  host-test source set is `androidHostTest`, per `KmpLibraryConventionPlugin`'s own comment) but
  not `androidHostTest` itself, so `FunctionNaming` flagged the test's backtick-name function.
  Added `**/androidHostTest/**` to all 14 occurrences of that excludes list (one per rule block).
  `PlatformComponentModule`'s JVM `actual` is an empty placeholder, mirroring the Android one — no
  JVM-specific platform bindings exist yet, matching `AppModule`'s doc comment. `WayprintDesktop.kt`
  skips Taiga's classloader-loaded `BitmapPainter` window icon (needs a `compose.components:resources`
  dependency Wayprint doesn't otherwise pull in, for a cosmetic default-icon replacement) — the
  nativeDistributions installer icon is unaffected, only the live window's taskbar icon is the
  JVM default rather than the logo.
  Verified end-to-end on this dev machine (real X display, not headless): `./gradlew build`,
  `detekt`, `ktlintCheck`, and `:composeApp:jvmTest` (including both `KoinGraphTest`s) all pass;
  `./gradlew :composeApp:run` launched a real window (screenshotted directly, not the full desktop)
  showing the Recents screen — "Wayprint" title, "No tracks yet — import a GPX to get started.",
  and the "Import GPX" FAB, proving Koin DI, `TracksStorage`, and the Compose UI all wire up
  correctly on JVM. Stopped short of exercising the file-picker/import/render/save flow interactively
  — the user flagged partway through that driving windows on their live desktop (unlike the
  isolated Android emulator) interferes with their own work, so this and any future Desktop-target
  step should stick to `./gradlew build`/tests plus a single non-interactive screenshot, not a
  multi-step manual click-through.
...
- [x] **M16.2** — Add the `iosApp` Xcode project scaffold at the repo root, mirroring
  ...
  **Verify (capped — no Mac in this environment):** `./gradlew :composeApp:linkDebugFrameworkIosSimulatorArm64`
  ...
  Note: the checklist's own guessed Verify command doesn't actually run here — Kotlin Gradle
  Plugin unconditionally disables Apple framework/binary **link** tasks (as distinct from the
  klib-producing `compileKotlinIosXxx` tasks M16.1 already verifies) on a non-macOS host; the
  task reports SKIPPED ("Task is enabled" is false) even with
  `-Pkotlin.native.enableKlibsCrossCompilation=true` forced on, with no known override. The
  achievable substitute, run instead: `./gradlew :composeApp:compileKotlinIosArm64
  :composeApp:compileKotlinIosSimulatorArm64` succeeds with the new `binaries.framework { baseName
  = "WayprintIos"; isStatic = true }` block added — confirming the framework export config is at
  least valid and doesn't break compilation, the same cap M16.1 already documented. Full
  `detekt`/`ktlintCheck`/`jvmTest`/`testFdroidDebugUnitTest` all still pass; `.editorconfig` needed
  a `[**/main.ios.kt]` override (`ktlint_standard_filename`/`ktlint_standard_function-naming`
  disabled) for the required PascalCase `MainViewController()` entry point, copied verbatim from
  `TaigaMobileNova/.editorconfig`'s existing precedent for the identically-named file. The
  `iosApp/` scaffold (`iosApp.xcodeproj/project.pbxproj`, `iOSApp.swift`, `ContentView.swift`,
  `Info.plist`, `Configuration/Config.xcconfig`, `Assets.xcassets` incl. `wayprint_logo.png` as the
  1024×1024 App Icon) mirrors Taiga's structure with fresh UUIDs and Wayprint's own bundle id
  (`com.grappim.wayprint.Wayprint$(TEAM_ID)`, `app-pkg` + product-name pattern, matching Taiga's
  analogous suffix). `composeApp`'s new `main.ios.kt` (`MainViewController()` via
  `ComposeUIViewController { startKoin<KoinApp> { }; WayprintAppContent() }`) omits Taiga's
  `NSLogLogger`/`setUnhandledExceptionHook`/`OnFocusBehavior` calls — Wayprint has no logger module
  and no established focus-behavior need yet, unlike Taiga. `xcuserdata`/`xcscheme` files (Taiga
  commits these, Xcode-generated on first open) were **not** fabricated — they're machine-specific
  state Xcode regenerates itself; only the actual project structure was scaffolded. This step is
  unverified end-to-end (no Mac here) beyond what's stated above — ask the user to open
  `iosApp.xcodeproj` on a real Mac, confirm it builds and runs on a simulator, before trusting M16
  as a whole.
